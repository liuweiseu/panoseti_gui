...
%thresh=245;obsmode='Imaging'
thresh=265;obsmode='Imaging'
% dataset=['skyzen_th' num2str(thresh)]
% direc=['C:\Users\jerome\Documents\panoseti\DATA\mtlaguna_quabo1\skyzen_th' num2str(thresh) '\'];
dataset=['skymoon_th' num2str(thresh)]
direc=['C:\Users\jerome\Documents\panoseti\DATA\mtlaguna_quabo1\skymoon_th' num2str(thresh) '\'];
...
format long
 ...
 nbnojump=1;
   ...
   if (nima==1) && (video==1) && (makeintro)==1
     text(0.3,0.5,'Sky Background','FontSize',28,'Color','b')
...
%    text(1,1,['Distance Lens-Detector [mm]: ' num2str(pos)],'Color','y','FontSize',12)
     text(1,1,['Sky, ' obsmode ', Th=' num2str(thresh) 'Gain=21'],'Color','y','FontSize',12)
...
%     
  ...
  imagesc( (1/nbim)* totima, ([Imin Imax]))
    axis image
%    text(1,1,['Distance Lens-Detector [mm]: ' num2str(pos)],'Color','y','FontSize',12)
     text(1,1,['Averaged Sky, ' obsmode ', Th=' num2str(thresh) 'Gain=21'],'Color','y','FontSize',12)
     %text(1,1.5,['Time comp:' datestr(timecomp(nima),'mmm.dd,yyyy HH:MM:SS.FFF') ' Packet No:' num2str(packetno(nima))],'Color','y','FontSize',12)
    cl=colorbar;
    cl.Label.String='Intensity';
    drawnow
  ...
  imagesc( (1/nbnojump)* totimanojump)
    axis image
%    text(1,1,['Distance Lens-Detector [mm]: ' num2str(pos)],'Color','y','FontSize',12)
     text(1,1,['Averaged Sky (no jumps), ' obsmode ', Th=' num2str(thresh) 'Gain=21'],'Color','y','FontSize',12)
     %text(1,1.5,['Time comp:' datestr(timecomp(nima),'mmm.dd,yyyy HH:MM:SS.FFF') ' Packet No:' num2str(packetno(nima))],'Color','y','FontSize',12)
    cl=colorbar;
    cl.Label.String='Intensity';
    drawnow
 ...
 %   plot(25.4*(24+1/8)+(1:numel(files))*0.8, nbpixsup3std,'+-')
  ...
  cla
  hold on
   plot((3600*24)*(timecomp-timecomp(1)), IntensmaxQ1,'-b')
      plot((3600*24)*(timecomp-timecomp(1)), IntenstotQ1,'--b')
         plot((3600*24)*(timecomp-timecomp(1)), IntensmaxQ2,'-r')
      plot((3600*24)*(timecomp-timecomp(1)), IntenstotQ2,'--r')
