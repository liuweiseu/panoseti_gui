...
IntensmeanQ9G=zeros(numel(dactab),numel(gaintabtab),numel(currtabExp));
 nboutlierlow=numel(1,dactab);
 nboutlierup=numel(1,dactab);
  ...
  %       pause(1)
      ...
      close(gcf)
            %what about outlier images?
             nboutlierlow(dd)=numel(find(spatialmeanvstime<0.7*mean(spatialmeanvstime)));
             nboutlierup(dd)=numel(find(spatialmeanvstime>1.3*mean(spatialmeanvstime)));
...
figure('Position',[10 10 1400 900])
hold on
plot(dactab,nboutlierlow,'+-')
plot(dactab,nboutlierup,'+-')


xlabel('DAC1 value')
ylabel([' Nb of bad packets (per ' num2str(nbimperdac) ' packets)'])
title(['Gain:' num2str(gaintab(1)) exptext ' No mask.  ' num2str(nbimperdac) ' images per DAC'])
...
%set(gca, 'YScale','log')
   ...
   yl=ylim;
        ...
        for pp=ceil(petab(1)):floor(petab(end))
              [minval minind] =  min(abs(dactab-(B0+A0*pp)));
            peaksdac3=[peaksdac3 dactab(minind(1))];
        plot([peaksdac3(end) peaksdac3(end)] , yl,'--')
         text(peaksdac3(end) ,-3+yl(2),[num2str(pp) 'pe'],'FontSize',16)
        end
        hold off
        
        legn=[{'Low median value over pixels'} {'High median value over pixels'}]
        legend(legn)
        
datenow=datestr(now,'yymmddHHMMSS');
saveas(gcf,[resultdir '\png\' 'Badframe_ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1))  '_' datenow '_nomask.png'])
saveas(gcf,[resultdir '\fig\' 'Badframe_ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])

set(gca,'YScale','log','YLim',[1e-1 1e3])
saveas(gcf,[resultdir '\png\' 'BadframeLOG_ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1))  '_' datenow '_nomask.png'])
saveas(gcf,[resultdir '\fig\' 'BadframeLOG_ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
...
set(gca, 'YScale','log')
datenow=datestr(now,'yymmddHHMMSS');
saveas(gcf,[resultdir '\png\' 'ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1))  '_' datenow '_nomask.png'])
saveas(gcf,[resultdir '\fig\' 'ADC_DACHOLDAll'  '_' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
...
for hhh=0:15
 ...
 set(gcf,'Color','w')
        ...
        %set(gca, 'YScale','log')
        datenow=datestr(now,'yymmddHHMMSS');
        saveas(gcf,[resultdir '\png\' 'ADC_DACLightHOLD' num2str(hhh) '_' 'Gain' num2str(gaintab(1))  '_' datenow '_nomask.png'])
        saveas(gcf,[resultdir '\fig\' 'ADC_DACLightHOLD' num2str(hhh) '_' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
end       
 ...
 htab=5*(0:15);
     ...
     %   text(dactab(2),yl(2)-5,['HOLD [ns]:' num2str(hhh*5)],'FontSize',18)
        ...
        %set(gca, 'YScale','log')
        datenow=datestr(now,'yymmddHHMMSS');
        saveas(gcf,[resultdir '\png\' 'ADC_HOLD1' num2str(hhh) '_' 'Gain' num2str(gaintab(1))  '_' datenow '_nomask.png'])
        saveas(gcf,[resultdir '\fig\' 'ADC_HOLD1' num2str(hhh) '_' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
