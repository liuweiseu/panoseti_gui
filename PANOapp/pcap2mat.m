function [capture cap] = pcap2mat(filter, decodeas_and_dissector, filename_or_interface, capture_stop_criteria,hkonly)
...
%     eval(['status=' os_cmd '(''tshark -i ' int2str(filename_or_interface) capture_stop_str ' -w tmp.pcap' capture_filter_str ''');'])
    eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -i ' int2str(filename_or_interface) capture_stop_str ' -w tmpnew.pcap' capture_filter_str ''');'])
...
if (~WSdissector_FLAG) % using MATLAB defined disssector
    ...
    if (capture_FLAG)
        eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -r ' read_filename ' -F k12text -w tmp.txt'');'])
    else
        eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -r ' read_filename ' -F k12text' read_filter_str ' -w tmp.txt'');'])
    end
    ...
    if (capture_FLAG)
        eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -r ' read_filename decodeas_str ' -T fields -E separator=' separator_char  WSdissector_str ' > tmp.txt '');'])
    else
        eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -r ' read_filename decodeas_str ' -T fields -E separator=' separator_char  WSdissector_str  capture_stop_str read_filter_str ' > tmp.txt '');'])
        if ~exist('hkonly','var')
            pcatxt2packets
        elseif hkonly==1
            pcatxt2packetshk
...
if exist('imags','var') 
    cap.images=imags;
    cap.boardloc=boardloc;
    cap.nanosec=nanosec;
    cap.acq_mode=acq_mode;
    cap.packet_no=packet_no;
elseif exist('hk','var')
     cap=hk;
else
    cap=[];
