...
%1Hz low thresh:
direc=[getuserdir '\panoseti\DATA\20190426\doublemobo4\'];
...
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%SECOND
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<300  & t1(1:end-1)>172);
 ind1h0=ind1h(1);
  indcyc2=ind1h;
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%THIRD
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<520  & t1(1:end-1)>424);
 ind1h0=ind1h(1);
  indcyc3=ind1h;
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%FOURTH
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<800  & t1(1:end-1)>690);
 ind1h0=ind1h(1);
  indcyc4=ind1h;
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%FIFTHTH
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<1400  & t1(1:end-1)>1000);
 ind1h0=ind1h(1);
  indcyc5=ind1h;
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 ylabel('Occurrence')
ti=title(['Flasher-blue on side-by-side mobos, all-pix-masked-excepted-one '])
set(ti,'FontSize',12)
...
legend('power cycle #4')
 ...
 subplot(3,2,5)
 histogram((1/3.2)*diffns5,'BinMethod','integers')
 xlabel('nanosec counter difference [3.2ns ticks]')
 ylabel('Occurrence')
legend('power cycle #5')
      
         datenow=datestr(now,'yymmddHHMMSS');
        saveas(gcf,[getuserdir '/panoseti/' 'sidebysidedoublemobos_masked' datenow '.png'])
        saveas(gcf,[getuserdir '/panoseti/' 'sidebysidedoublemobos_' datenow '.fig'])
