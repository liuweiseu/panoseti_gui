...
def makeim(sock,fullfilename,nbimmax):
    ...
    while nbim<nbimmax:
        ...
        fhand.write ("CompTime=" + now + ", acqmode= " + str(bytesback[0])+ ", pktno = " + str(pktno) +", nanosec=" + str(nanosec)+", utc=" + str(utc)+  ", bdloc= " + hex(bytesback[5]*256 +bytesback[4]) + "\n")
...
def makemean(sock,fullfilename,nbimmax):
    #print ("Quadrant Board Data Logger")
    nbim = 0 
    sum = np.zeros(256)
    #last_ET = 0;
    ...
    while nbim<nbimmax:
        ...
        #print(str(nbim))
        #pyfits.append('test2.fits',np.array(chip4))
    mean = np.true_divide(sum, nbimmax)
    return mean