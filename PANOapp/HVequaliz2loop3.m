...
Temp125=[Temp125 Tnow];

meth=0
optimal25HV0=54.195;optimal25HV1=54.196;optimal25HV2=54.194;optimal25HV3=54.204;

optimalHV0=optimal25HV0+(0.054*(Tnow-25));optimalHV1=optimal25HV1+(0.054*(Tnow-25));optimalHV2=optimal25HV2+(0.054*(Tnow-25));optimalHV3=optimal25HV3+(0.054*(Tnow-25));
maxhv=60.1;
resQ=0.002;%resQ=5;
  ...
  % pause(6)
   ...
   hk = getlastHK;
    ...
    if (hk.hvmon0< maxhv) || (hk.hvmon1< maxhv) || (hk.hvmon2< maxhv) || (hk.hvmon3< maxhv)
        ...
        changeHV0=0;
        if (abs(costQ0)>resQ)
            newHV0=HVrV_0-sign(costQ0)*abs(costQ0);%*HVinc
            changeHV0=1;
        end
        changeHV1=0;
        if (abs(costQ1)>resQ)
            newHV1=HVrV_1-sign(costQ1)*abs(costQ1);%*HVinc;
            changeHV1=1;
        end
        changeHV2=0;
        if (abs(costQ2)>resQ)
            newHV2=HVrV_2-sign(costQ2)*abs(costQ2);%*HVinc;
            changeHV2=1;
        end
        changeHV3=0;
        if (abs(costQ3)>resQ)
            newHV3=HVrV_3-sign(costQ3)*abs(costQ3);%*HVinc;
            changeHV3=1;
        end
        ...
        if isnumeric(newHV0) && (newHV0>0) && (newHV0<maxhv) && ...
            ...
            %quaboconfig(index,2)=string(floor( ( optimalHV0 +1.073)/0.0011453    ));
             quaboconfig(index,2)=string(floor( ( newHV0 +1.073)/0.0011453    ));
            [ia,index]=ismember('HV_1 ',quaboconfig);
            %quaboconfig(index,2)=string(floor(    (1/(Vref*(10/(65536*158))))*newHV1     ));
             %quaboconfig(index,2)=string(floor( ( optimalHV1 +1.073)/0.0011453    ));
              quaboconfig(index,2)=string(floor( ( newHV1 +1.073)/0.0011453    ));
            [ia,index]=ismember('HV_2 ',quaboconfig);
            %quaboconfig(index,2)=string(floor(    (1/(Vref*(10/(65536*158))))*newHV2     ));
            % quaboconfig(index,2)=string(floor( ( optimalHV2 +1.073)/0.0011453    ));
              quaboconfig(index,2)=string(floor( ( newHV2 +1.073)/0.0011453    ));
            [ia,index]=ismember('HV_3 ',quaboconfig);
           % quaboconfig(index,2)=string(floor(    (1/(Vref*(10/(65536*158))))*newHV3     ));
           %quaboconfig(index,2)=string(floor( ( optimalHV3 +1.073)/0.0011453    ));
            quaboconfig(index,2)=string(floor( ( newHV3 +1.073)/0.0011453    ));
...
disp(['Temp125[\circC] Now: ' num2str(Temp125(end)) ' previous iter was:' num2str(Temp125(end-1>0))])
disp(['Requested HV Q0[V]:' num2str(optimalHV0) ' Q1[V]:' num2str(optimalHV1) ...
    ' Q2[V]:' num2str(optimalHV2) ' Q3[V]:' num2str(optimalHV3)])

 sendHV(quaboconfig)
            end
            disp(['Real HV Q0[V]:' num2str(hk.hvmon0) ' Q1[V]:' num2str(hk.hvmon1) ...
    ' Q2[V]:' num2str(hk.hvmon2) ' Q3[V]:' num2str(hk.hvmon3)])
disp(['Optimal HV Q0[V]:' num2str(optimalHV0) ' Q1[V]:' num2str(optimalHV1) ...
    ' Q2[V]:' num2str(optimalHV2) ' Q3[V]:' num2str(optimalHV3)])
        end
...
% subplot(1,3,1)
% hold on
% plot(cpsfac*IntensmeanQ0,'b')
% plot(cpsfac*IntensmeanQ1,'r')
% plot(cpsfac*IntensmeanQ2,'g')
% plot(cpsfac*IntensmeanQ3,'m')
% hold off
% xlabel('iter#')
