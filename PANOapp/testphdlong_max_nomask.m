...
%dactab=[280];
%dactab0=dactab;
nbimperdac=100;timemax=30; %15.6*3600;
gaintab=33; usegainmap=1;
petab=[10.5]; % threshold for acquisitions
...
if usegainmap==0
    ...
    load(['pethresh_gain' num2str(gaintab) '.mat'])
%     petab0=(1/A0)*(-B0 + dactab); % A0...A3
%     indcoldac=1; %%% 1...4 attention!
    load(['gainmap_gain' num2str(gaintab) '.mat'])
        for pp=0:63
             [ig,indexgain]=ismember(['GAIN' num2str(pp)] ,quaboconfig);
              testpixelxy1=squeeze(marocmap(pp+1,1,:));
              testpixelxy2=squeeze(marocmap(pp+1,2,:));
              testpixelxy3=squeeze(marocmap(pp+1,3,:));
              testpixelxy4=squeeze(marocmap(pp+1,4,:));
              gainpix1=(dec2hex(gainmap(testpixelxy1(1),testpixelxy1(2))));
              quaboconfig(indexgain,1+1)={['0x' gainpix1]};
              gainpix2=(dec2hex(gainmap(testpixelxy2(1),testpixelxy2(2))));
              quaboconfig(indexgain,2+1)={['0x' gainpix2]};
              gainpix3=(dec2hex(gainmap(testpixelxy3(1),testpixelxy3(2))));
              quaboconfig(indexgain,3+1)={['0x' gainpix3]};
              gainpix4=(dec2hex(gainmap(testpixelxy4(1),testpixelxy4(2))));
              quaboconfig(indexgain,4+1)={['0x' gainpix4]};
        end
   end
...
dactab2=A2*petab+B2;
dactab3=A3*petab+B3;
dactab=round([dactab0 dactab1 dactab2  dactab3]);
for iq=0:3
  quaboconfig(indexdac,iq+1+1)={['0x' dec2hex(dactab(iq+1))]};
end

            disp(['Sending Maroc comm...DAC1:' num2str(dactab(dd)) ' Gain:' num2str((gaintab)) ])
