...
dataset='Procyon2'
direc=['C:\Users\jerome\Documents\panoseti\DATA\mtlaguna_quabo1\procyon_ima2\'];
...
%     
 ...
 %   plot(25.4*(24+1/8)+(1:numel(files))*0.8, nbpixsup3std,'+-')
  ...
  %  ylabel(['Nb pix with I > '  'mean(noise)+ 10 sigma(noise) '],'FontSize',16)
    ...
    saveas(gcf,[dataset 'P2sup_ima' num2str(ii) '.png'])
   saveas(gcf,[dataset 'P2sup_ima' num2str(ii) '.fig'])
  ...
  hold on
   ...
   %xlim(25.4*(24+1/8)+0.8*[1 numel(files)])
    ...
    xlabel('Time (s)','FontSize',22)
    legend('Imax','Itotal')
       saveas(gcf,[dataset 'P2intens_ima' num2str(ii) '.png'])
   saveas(gcf,[dataset 'P2intens_ima' num2str(ii) '.fig'])
