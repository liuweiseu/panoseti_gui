function [images nbimafin key key2 key3]=grabimages2key(nim,mode,rec)   
if mode==1
    recorddata=1;
            ...
            while  (dolisten==1) && (missedpac<app.fitsmax+1)
                ...
                if (recorddatafast==1) || (displaymean ==1)
                    ...
                    if  recorddataudpnodisp==1 && (~exist('u','var') || strncmp(u.Status,'clo',3))
                        u = udp('192.168.0.4',porthost, 'LocalPort', port,'InputBufferSize',528*8,'DatagramTerminateMode','on');
                    ...
                    try
                        ...
                        if  recorddataudpnodisp==1
                            ...
                            [packet,SOURCEHOST]  = judp('RECEIVE',port,2048, 20000);
                        end
                    catch
                        packet=[];
                      %  updateconsoleapp(app,'No packet received.')
...
key(app.fitsnum)=nanosec;

                            %%save data
                            
                            timenow=now;
%key2(app.fitsnum)=timenow;                      
key2(app.fitsnum)=packet_no;
key3(app.fitsnum)=acq_mode;
