...
%offsets = zeros(1, 1 + num_extensions);%JM mod
offsets = zeros(1, num_extensions);

offsets(1) = info.PrimaryData.Offset;

for j = 1:num_images
    %offsets(j+1) = info.Image(j).Offset;%JM mod
    offsets(j) = info.Image(j).Offset;
end
n = 1 + num_images;
...
switch(hdu_type)
    case 'image'
        infoind=zeros(1,numel(index));
       for ii=1:numel(index)
           infoind(ii)=info.Image(index).Offset;
       end
        hdu_index = find(infoind == offsets);
    case 'ascii_table'
