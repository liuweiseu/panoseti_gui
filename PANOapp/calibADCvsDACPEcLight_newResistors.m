...
calibdir='C:\Users\jerome\Documents\panoseti\tmpres\';
...
usegainmap=1;
gaintabtab=[30 60 90 120 160 200 240];%11:31 %1:10:121;
petab=[4 8 12];
% currtabExp=[0. 2.^(5:2:11)];
currtabExp=[0:1000:10000];
...
exptext=' Background Light ON. ' %' Light ON (400uA) '
%currtabExp=[0. ];
maxtesttime = 60;
               ...
               % mxL=32; my= 3;
                ...
                while dd<=numel(dactab)
                    ...
                    while (nbtot<nbimperdac) && (nbimafin~=0)
                        [imageszerostmp nbimafin]=grabimages3(nbimperdac,1,1);
                    ...
                    if (nbimafin~=0)
                        ...
                        if sum(images,'all')>0
                            ...
                            IntenspixGen(dd,gainkk,my,cc)=meanimage(xt1,yt1);
                            nbtrig=nbtrig+1
...
load([getuserdir '\panoseti\Calibrations\' 'LEDND_Brightness_current_' '191017153627' '.mat'])
...
for ig =1:numel(gaintabtab)
    ...
    hold on
    for cc=1:numel(currtabExp)
        plot(petab,IntenspixGen(:,ig,1,cc))
    end
    xlabel('pe#')
    ylabel('ADC value')
    hold off
   ti= title(['Gain ini:' num2str(gaintabtab(ig)) ', all-pix-masked-exc.-one, flood-illumination, ' 'Q' num2str(0)  ])
      set(ti,'FontSize',10)
      
    leg={};
    for cc=1:numel(currtabExp)
        leg(cc)={ ['Light [nW/pix]:' num2str(powerdet(cc))]};
    end
    legen=legend(leg,'Location','SouthWest')
    set(legen,'FontSize',12,'Color','none')
    box on
    grid on
    ...
    hold on
    for cc=1:numel(currtabExp)
        plot(petab,IntenspixGen(:,ig,2,cc))
    end
    hold off
    title(['Q' num2str(1)])
    xlabel('pe#')
    ...
    hold on
    for cc=1:numel(currtabExp)
        plot(petab,IntenspixGen(:,ig,3,cc))
    end
    hold off
    title(['Q' num2str(2)])
    xlabel('pe#')
    ...
    legen=legend(leg,'Location','SouthWest','Color','none')
    set(legen,'FontSize',12)
    box on
    grid on
    ...
    hold on
    for cc=1:numel(currtabExp)
        plot(petab,IntenspixGen(:,ig,4,cc))
    end
    hold off
    title(['Q' num2str(3)])
    xlabel('pe#')
    ...
    datenow=datestr(now,'yymmddHHMMSS');
    saveas(gcf,[calibdir 'LightADC_DAC_gain' num2str(gaintabtab(ig)) '_' datenow '_masked.png'])
    saveas(gcf,[calibdir 'LightADCvsDAC_gain' num2str(gaintabtab(ig)) '_'    datenow '_masked.fig'])
...
for th =1:numel(petab)
    ...
    ylabel('ADC value')
    hold off
    ti=title(['Threshold [pe#]: ' num2str(petab(th)) ', all-pix-masked-exc.-one, flood-illumination'  ', Q' num2str(0)])
    set(ti,'FontSize',10)
    leg={};
    for ii=1:numel(gaintabtab)
        leg(ii)={ ['Gain: ' num2str(gaintabtab(ii))]};
    end
    legend(leg,'Location','SouthWest','Color','none')
    box on
    grid on
    ...
    hold on
    for ig=1:numel(gaintabtab)
        plot(powerdet,squeeze(IntenspixGen(th,ig,2,:)),'-+')
    end
    hold off
    title(['Q' num2str(1)])
    xlabel('Light Intensity [nW/pix]')
    ...
    hold on
    for ig=1:numel(gaintabtab)
        plot(powerdet,squeeze(IntenspixGen(th,ig,3,:)),'-+')
    end
    hold off
    title(['Q' num2str(2)])
    xlabel('Light Intensity [nW/pix]')
    ...
    hold on
    for ig=1:numel(gaintabtab)
        plot(powerdet,squeeze(IntenspixGen(th,ig,4,:)),'-+')
    end
    hold off
    title(['Q' num2str(3)])
    xlabel('Light Intensity [nW/pix]')
    ...
    datenow=datestr(now,'yymmddHHMMSS');
    saveas(gcf,[calibdir 'LightADC_DAC_th' num2str(th) '_'   datenow '_masked.png'])
    saveas(gcf,[calibdir 'LightADCvsDAC_'  num2str(th) '_'  datenow '_masked.fig'])
    
end

save([calibdir 'Lighttmp'  '_'   datenow '.mat'])
