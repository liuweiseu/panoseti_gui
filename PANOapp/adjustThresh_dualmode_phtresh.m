...
quaboconfig(indexacqint,3)=expo-1;
...
[ig,indexacqmode]=ismember(['ACQMODE '] ,quaboconfig);
    quaboconfig(indexacqmode,2)={"0x03"};
...
[ia,indexdac2]=ismember('DAC1',quaboconfig);
indcol=1:4;
quaboconfig(indexdac2,indcol+1)={['0x' dec2hex(dacIma)]};
...
for dd=1:numel(dactab)
 ...
 disp(['Sending Maroc comm...DAC2:' num2str(dactab(dd))])
sendconfig2Maroc(quaboconfig)
...
nima=numel(indima);
    ...
    allnima(dd)=nima;
    disp([num2str(dd) '/' num2str(numel(dactab)) ' dacPH:' num2str(dactab(dd))...
...
text(0,23,[num2str(dd) '/' num2str(numel(dactab)) ' dacIma:' num2str(dactab(dd))...
    ...
    plot(nano(indima));ylabel('Ima Nanosec')
     subplot(3,2,4)
cla;
    hold on; plot(packnum(indima));plot(packnum(indph),'r');legend('Ima','PH'),ylabel('Packet num')
     drawnow 
...
cla;
    hold on; plot(packimadiff,'-+');plot(packphdiff,'r');legend('Ima','PH'),ylabel('Packet num DIFF')
     drawnow  
...
%     
    ...
    IntensmeanQ3=[IntensmeanQ3 mean(meanimage(9:16,9:16),[1 2])];
        %   IntensmaxQ4(nima)=max(ima(9:16,9:16),[],'all');
    IntensmeanQ8=[IntensmeanQ8 (meanimage(7,9))];
...
normima1=(1e-5*expo).^(-1);
normima=(1e-5*expo*allnima).^(-1);

 plot(dactab, normima1.*IntensmeanQ8,'-+b')
  plot(dactab, normima.*allnph,'-+r')
  legend(['Imaging mode '  '(DAC1=' num2str(dacIma) ')'], ['simultaneous PH mode'])
...
title(['Simultaneous IMA/PH; Gain:' num2str(gain) 'd(' dec2hex(gain) 'h); all-pixels-masked-excepted-one '])
 %datenow=datestr(now,'yymmddHHMMSS');
  saveas(gcf,['threshPHsweepIma' num2str(dacIma) '_' datenow '_masked.png'])
    saveas(gcf,['threshPHsweepIma' num2str(dacIma) '_' datenow '_masked.fig'])
