...
allADCpixt=squeeze(images(xt1,yt1,:));
%%%PHD
minp=min(allADCpixt);
maxp=max(allADCpixt);
factbin=10;
...
ylabel(['Pulse counts per sec over threshold (DAC1=' num2str(dactab0) ')'])
box on
title(['hold [ns]:' num2str(hh*5) ' Gain:' num2str(gaintab(1)) exptext ' All-pix-masked-exc.-one. ' num2str(nbimperdac) ' trigger events'],'FontSize',12)
        ...
        for pp=1:numel(petab)
            ppp=petab(pp);
               ...
               % ADCDACfitPH= petab(minind);
                ...
                text(ADCDACfitPH(pp) ,1e2,[num2str(ppp) 'pe'],'FontSize',16)
        end
        hold off
