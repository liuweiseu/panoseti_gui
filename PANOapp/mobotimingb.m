...
%1Hz low thresh:
direc=[getuserdir '\panoseti\DATA\20190426\doublemobo3\'];
...
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%SECOND
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<200  & t1(1:end-1)>133);
 ind1h0=ind1h(1);
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%THIRD
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<400  & t1(1:end-1)>300);
 ind1h0=ind1h(1);
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 for oo=1:numel(ind1h)
  ...
  %%%FOURTH
 ind1h=find((abs(tdiff-1)<0.35) & t1(1:end-1)<690  & t1(1:end-1)>450);
 ind1h0=ind1h(1);
% ind1hz=find((abs(tdiff-floor(tdiff))<0.25) & tdiff>0.5 & t1<220);
 ...
 ylabel('Occurrence')
title(['Flasher-blue on side-by-side mobos '])
...
legend('power cycle #4')
        ...
        saveas(gcf,[getuserdir '/panoseti/' 'sidebysidedoublemobos_' datenow '.png'])
