...
acqint=(1e-6)*(str2num(quaboconfig(indexacqint,2))+1);
...
for igg=1:numel(gaintaballg)
    ...
    allima=zeros(16,16,numel(dactab));
    %indcol2=1;
    ...
    ylim(yl)
         ...
         saveas(gcf,[calibdir 'pix2pixgainInit' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.jpg'])
     saveas(gcf,[calibdir 'pix2pixgainInit' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
                      
    
    
    %%%% acquire with mask to get DAC offset
    %  maskmode=0;
    % quaboconfig = changemask(maskmode,[],quaboconfig); %maskmode: unmask all pix (=0); mask all excepted some (=1)
    % TBDone: better determine which pixel is used (mid curve and not hot pixel)
    %  quaboconfig = changemask(1,[2 1;2 2; 2 3 ; 2 4],quaboconfig); %maskmode: unmask all pix (=0); mask all excepted some (=1)
    quaboconfig = changemask(1,[marocmap16(refpixelxyQ0(1),refpixelxyQ0(2),:);marocmap16(refpixelxyQ1(1),refpixelxyQ1(2),:); ...
        marocmap16(refpixelxyQ2(1),refpixelxyQ2(2),:) ;marocmap16( refpixelxyQ3(1),refpixelxyQ3(2),:)],quaboconfig); %maskmode: unmask all pix (=0); mask all excepted some (=1)
    
    dactaboffset=dactab(1):210;
    allimadacoffset=zeros(16,16,numel(dactaboffset));
    
    %indcol2=1;
    for dd=1:numel(dactaboffset)
        quaboconfig(indexdac,indcol+1)={['0x' dec2hex(dactaboffset(dd))]};
        disp(['Sending Maroc comm...DAC1:' num2str(dactaboffset(dd)) '/' num2str(dactaboffset(end)) ' Gain' num2str(gaintab) ' iter' num2str(loopgaincnt)  ])
        sendconfig2Maroc(quaboconfig)
        ...
        meanimage=mean(images(:,:,:),[3])';
        allimadacoffset(:,:,dd)=meanimage;
    end
    ophdrefQ0=squeeze(allimadacoffset(refpixelxyQ0(1),refpixelxyQ0(2),:));
    ophdrefQ1=squeeze(allimadacoffset(refpixelxyQ1(1),refpixelxyQ1(2),:));
    ophdrefQ2=squeeze(allimadacoffset(refpixelxyQ2(1),refpixelxyQ2(2),:));
    ophdrefQ3=squeeze(allimadacoffset(refpixelxyQ3(1),refpixelxyQ3(2),:));
    
    [val indmax0]=max(ophdrefQ0);
    dacoffset0=dactaboffset(indmax0(1));
    [val indmax1]=max(ophdrefQ1);
    dacoffset1=dactaboffset(indmax1(1));
    [val indmax2]=max(ophdrefQ2);
    dacoffset2=dactaboffset(indmax2(1));
    [val indmax3]=max(ophdrefQ3);
    dacoffset3=dactaboffset(indmax3(1));
    disp(['DAC offsets: Q0: ' num2str(dacoffset0) ' ,Q1:' num2str(dacoffset1) ' ,Q2:' num2str(dacoffset2) ' ,Q3:' num2str(dacoffset3)])
    figure
    subplot(2,2,1)
    plot(dactaboffset, normcps * squeeze(allimadacoffset(refpixelxyQ0(1),refpixelxyQ0(2),:)),'-o','Color',[0.5 0.5 0.5],'LineWidth',3.5)
    subplot(2,2,2)
    plot(dactaboffset, normcps * squeeze(allimadacoffset(refpixelxyQ1(1),refpixelxyQ1(2),:)),'-o','Color',[0.5 0.5 0.5],'LineWidth',3.5)
    subplot(2,2,3)
    plot(dactaboffset, normcps * squeeze(allimadacoffset(refpixelxyQ2(1),refpixelxyQ2(2),:)),'-o','Color',[0.5 0.5 0.5],'LineWidth',3.5)
    subplot(2,2,4)
    plot(dactaboffset, normcps * squeeze(allimadacoffset(refpixelxyQ3(1),refpixelxyQ3(2),:)),'-o','Color',[0.5 0.5 0.5],'LineWidth',3.5)
         saveas(gcf,[calibdir 'pix2pixDACoffset' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.jpg'])
     saveas(gcf,[calibdir 'pix2pixDACoffset' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
                      
    
    %%%%%
    %         determine dacxpe DAC at x pe (fixed for uniformity between quabos)
    refpe=4.5;pedacoffset=12.*(refpe-0.5); %(should have gain=60)
    threshxpe=300;
    epscps=100;
    gainloopmax=60;
    
    dacxpe0=dacoffset0+pedacoffset ; %dac at 2.5pe
    dacxpe1=dacoffset1+pedacoffset ; %dac at 2.5pe
    dacxpe2=dacoffset2+pedacoffset ; %dac at 2.5pe
    dacxpe3=dacoffset3+pedacoffset ; %dac at 2.5pe
    %acquisitions at dacref:
    quaboconfig(indexdac,1+1)={['0x' dec2hex(dacxpe0)]};
    quaboconfig(indexdac,2+1)={['0x' dec2hex(dacxpe1)]};
    quaboconfig(indexdac,3+1)={['0x' dec2hex(dacxpe2)]};
    quaboconfig(indexdac,4+1)={['0x' dec2hex(dacxpe3)]};
    maskmode=0;
    quaboconfig = changemask(maskmode,[],quaboconfig); %maskmode: unmask all pix (=0); mask all excepted some (=1)
    disp(['Sending Maroc comm...DAC1:' num2str(dactaboffset(dd)) '/' num2str(dactaboffset(end)) ' Gain' num2str(gaintab) ' iter' num2str(loopgaincnt)  ])
    sendconfig2Maroc(quaboconfig)
    pause(pausetime)
    images=grabimages(nim,1,0);
    meanimagenorm=normcps * mean(images(:,:,:),[3])';
   ...
   % inddetrow=find(quaboDETtable(:,1)==str2num(cell2mat(quaboconfig(indexquabosn,3))));
    ...
    %find bad pixels, will be stored and masked
    threshbadpix=1e5;
    [Xbadpix Ybadpix]=find(meanimagenorm>threshbadpix);
    %%badpix=cell(1,30)
    if size(Xbadpix,1)>0
        if exist([getuserdir filesep 'panoseti' filesep 'Calibrations' filesep 'badpixmaps.mat'],'file')
     load((([getuserdir filesep 'panoseti' filesep 'Calibrations' filesep 'badpixmaps.mat'])),'badpix')
        else
            badpix={};
        end
     badpix(inddetrow)={[Xbadpix Ybadpix]};
    save((([getuserdir filesep 'panoseti' filesep 'Calibrations' filesep 'badpixmaps.mat'])),'badpix')
    end
    %         for each pix
    %             while abs(phdpix - phdref(dacxpe))>eps
    %                 change gain
    %                 acquisitions at dacref
    %             end
    %         end
    %         eventually redo full cps curves
    ...
    %    for jj=1:16
            ...
            disp(['testing Pix [' num2str(ii) ',' num2str(jj) '] with different gain values '])
            res=( meanimagenorm-threshxpe);
            ctgainloop=1;peakmoveQ0=zeros(16,16);
            while (max(abs(res),[],'all')>epscps) && (ctgainloop<gainloopmax)
                 (meanimagenorm(ii,jj)<threshbadpix)
                ctgainloop=ctgainloop+1;
                peakmoveprevious=peakmoveQ0;
                peakmoveQ0=sign(res);
                %don't change if ok:
                indok=find(res<epscps)
                peakmoveQ0(indok)=0;
                %don't change if bad pixels:
               if  size(Xbadpix,1)>0
                   for bb=1:size(Xbadpix,1)
                       peakmoveQ0(Xbadpix(bb),Ybadpix(bb))=0;
                   end
               end
               %don't change if converged:
               indconv= find(peakmoveprevious+peakmoveQ0==0);
                peakmoveQ0(indconv)=0;
                %get old gain and increment it
%                 [ig,indexgain]=ismember(['GAIN' num2str(marocmap16(ii,jj,1)-1)] ,quaboconfig);
%                 gainpixold=char(quaboconfig(indexgain,marocmap16(ii,jj,2)+1));
%                 gainpixnew=dec2hex(hex2dec(gainpixold(3:end))-peakmoveQ0);
                
                gainmap=gainmap-peakmoveQ0;
                
                for ii=1:16
                    for jj=1:16
                         [ig,indexgain]=ismember(['GAIN' num2str(marocmap16(ii,jj,1)-1)] ,quaboconfig);
                quaboconfig(indexgain,marocmap16(ii,jj,2)+1)={['0x' dec2hex(gainmap(ii,jj))]};
                    end
                end
                sendconfig2Maroc(quaboconfig)
                pause(pausetime)
                if sum(peakmoveprevious+peakmoveQ0,'all')~=0
                    images=grabimages(100,1,0);
                    
                    meanimage=mean(images(:,:,:),[3])';
                    res=(normcps *meanimage-threshxpe)
                else
                    ctgainloop=gainloopmax;
                end
            end
        %end
    %end
    ...
    figure
    imagesc(meanimage)
         saveas(gcf,[calibdir 'FinalImage' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.jpg'])
     saveas(gcf,[calibdir 'FinalImage' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
                      
    %%%do final cps curve (just to check, not necessary)
    figure
    allimaf=zeros(16,16,numel(dactab));
    %indcol2=1;
    for dd=1:numel(dactab)
        quaboconfig(indexdac,indcol+1)={['0x' dec2hex(dactab(dd))]};
        disp(['Sending Maroc comm...DAC1:' num2str(dactab(dd)) '/' num2str(dactab(end))  ' iter final'   ])
        sendconfig2Maroc(quaboconfig)
    ...
    finalfig=figure('Position',[50 50 1600 900])
    hold on
    subplot(2,2,1)
    hold on
    for ii=1:8
        for jj=9:16
            plot(dactab, normcps * squeeze(allimaf(jj,ii,:)),'-+','Color',[0.5 ii/8 jj/16])
        end
    end
    hold off
    title(['Q0 Bipolar fs; Gain:' num2str(gaintab) 'd(' dec2hex(gaintab) 'h); ' masklabel])
    ...
    for ii=1:8
        for jj=1:8
            ...
            plot(dactab, normcps *  squeeze(allimaf(jj,ii,:)),'-+','Color',[0.5 ii/8 jj/8])
        end
    end
    hold off
    title('Q1 all pix')
    ...
    for ii=9:16
        for jj=1:8
            plot(dactab, normcps *  squeeze(allimaf(jj,ii,:)),'-+','Color',[0.5 ii/16 jj/8])
        end
    end
    hold off
    title('Q2 all pix')
    ...
    for ii=9:16
        for jj=9:16
            plot(dactab, normcps *  squeeze(allimaf(jj,ii,:)),'-+','Color',[0.5 ii/16 jj/16])
        end
    end
    hold off
    title('Q3 all pix')
    ...
    xlim([dactab(1) dactab(end)])
     ...
     saveas(gcf,[calibdir 'pix2pixgainFinal' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.jpg'])
     saveas(gcf,[calibdir 'pix2pixgainFinal' 'Gain' num2str(gaintab(1)) '_' datenow '_nomask.fig'])
                      
    
    %% 1)  use 0 and y pe levels
    %%%%% get values for ref pe
    % on veut trouver la pente et intercept de dactab = A pe + B
    %% a refpe pe on a dacref= refpe*A +B ; a 5pe on a dac5 = 5A + B; a 0pe on a dac =B
    %% (2)-(1) gives A =(1/3) * (dac5-dac2) et B = dac2-(2/3)*(dac5-dac2) = (5/3)*dac2 -(2/3)*dac5
    An=zeros(1,4);
    Bn=zeros(1,4);
    
    An(:) = (1/gaintab(1)) * (1/refpe) * (pedacoffset);
    Bn(1) =  dacoffset0;
    Bn(2) =  dacoffset1;
    Bn(3) =  dacoffset2;
    Bn(4) =  dacoffset3;
    ...
    savedata=1;
    if savedata==1
        quaboDETtable(inddetrow,7)=An(1);
        quaboDETtable(inddetrow,8)=An(2);
        quaboDETtable(inddetrow,9)=An(3);
        quaboDETtable(inddetrow,10)=An(4);
        quaboDETtable(inddetrow,11)=Bn(1);
        quaboDETtable(inddetrow,12)=Bn(2);
        quaboDETtable(inddetrow,13)=Bn(3);
        quaboDETtable(inddetrow,14)=Bn(4);
...
%save gain map
load([getuserdir filesep 'panoseti' filesep 'Calibrations' filesep 'gainmap_inc.mat'],'gainmapallgmeanSN')
        gainmapallgmeanSN(:,:,inddetrow)=(1/gaintab(1))*(gainmap-gaintab(1));
