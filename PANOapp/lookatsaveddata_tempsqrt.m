 ...
 IntenstotQ4=zeros(1,nbim)+nan;
   ...
   if (nima==1) && (video==1) && (makeintro)==1
     ...
     axis off
         ...
         filename=['dataset_' dataset '_'  num2str(ii) 'sqrt.gif'];
   ...
   %  subplot(1,3,1)
    imagesc(  sqrt(images(:,:,nima)), sqrt([Imin Imax]))
    axis image
%    text(1,1,['Distance Lens-Detector [mm]: ' num2str(pos)],'Color','y','FontSize',12)
    ...
    cl=colorbar;
    cl.Label.String='sqrt(Intensity)';
    drawnow
...
%     
 ...
 %   plot(25.4*(24+1/8)+(1:numel(files))*0.8, nbpixsup3std,'+-')
  ...
  %  ylabel(['Nb pix with I > '  'mean(noise)+ 10 sigma(noise) '],'FontSize',16)
    ...
    saveas(gcf,['sup_ima' num2str(ii) 'sqrt.png'])
   saveas(gcf,['sup_ima' num2str(ii) 'sqrt.fig'])
  ...
  hold on
   ...
   %xlim(25.4*(24+1/8)+0.8*[1 numel(files)])
    ...
    xlabel('Time (s)','FontSize',22)
    legend('Imax','Itotal')
       saveas(gcf,['intens_ima' num2str(ii) 'sqrt.png'])
   saveas(gcf,['intens_ima' num2str(ii) 'sqrt.fig'])
  ...
  hold on
   ...
   %xlim(25.4*(24+1/8)+0.8*[1 numel(files)])
    ...
    legend('Imax Q1','Itotal Q1','Imax Q2','Itotal Q2','Imax Q3','Itotal Q3','Imax Q4','Itotal Q4')
       saveas(gcf,['intensQ_ima' num2str(ii) 'sqrt.png'])
   saveas(gcf,['intensQ_ima' num2str(ii) 'sqrt.fig'])
end
