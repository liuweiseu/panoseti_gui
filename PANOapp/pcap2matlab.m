...
if (~WSdissector_FLAG) % using MATLAB defined disssector
    ...
    if (capture_FLAG)
        ...
        eval(['status=system(''C:\PROGRA~1\Wireshark\tshark -r ' read_filename decodeas_str ' -T fields -E separator=' separator_char  WSdissector_str  read_filter_str ' > tmp.txt '');'])
    end
    assert(~status,'Reading capture using Tshark did not run well. Please make sure your inputs were correct.')
end
