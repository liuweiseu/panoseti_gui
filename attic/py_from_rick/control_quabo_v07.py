...
while True:
    ...
    "SHO" to open shutter,
    "SHC" to close shutter,
    "FAN" to change fan speed,
    "WR" to send White Rabbit shell command,
    ...
    if inp == 'q':
      ...
      #while(1):
        ...
        inp = input("Input # steps, -32768 to +32767\n\r")
        steps = int(inp)
      #while(1):
        cmd_payload = bytearray(64)
        for i in range(64): cmd_payload[i]=0
        ...
        inp = input("Enter WR shell command, or 'erase flash', or 'setup flash', or 'init flash', or Enter for escape \n")
        if inp == '':    #Send escape character to exit 'gui' function
            cmd_payload = bytearray(64)
            cmd_payload[0] = 0x30
            cmd_payload[1] = 0x30
            cmd_payload[2] = 0x1b
            #flush_rx_buf()
            sendit(cmd_payload)
        elif inp == 'erase flash':
            inp = '00' + 'sdb fse 0 31457280 256\r'  #Ascii '0' is 0x30
            cmd_payload = bytearray()
            cmd_payload.extend(inp.encode('utf-8'))           
            #Need to make sure the payload is an even number of bytes!
            if (len(cmd_payload) & 0x1): cmd_payload.append(0x20)                   
            sendit(cmd_payload)
        elif inp == 'setup flash':
            inp = '00' + 'sdb fs 0 31457280 256\r'  #Ascii '0' is 0x30
            cmd_payload = bytearray()
            cmd_payload.extend(inp.encode('utf-8'))           
            #Need to make sure the payload is an even number of bytes!
            if (len(cmd_payload) & 0x1): cmd_payload.append(0x20)       
            ...
            inp = '00' + 'sfp add AXGE-1254-0531 180750 148326 72169888\r'  #Ascii '0' is 0x30
            cmd_payload = bytearray()
            cmd_payload.extend(inp.encode('utf-8'))           
            #Need to make sure the payload is an even number of bytes!
            if (len(cmd_payload) & 0x1): cmd_payload.append(0x20)                   
