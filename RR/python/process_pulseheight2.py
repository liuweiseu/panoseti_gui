...
def put_channels_in_order(fhand, alist):
	for n in range(256):
		if remap_array[n] == 255: continue
		fhand.write (str(alist[remap_array[n]]) + ",")
		
	
pathname = '.\data'
remap_array = []
for n in range(256): remap_array.append(0)
try:
    inhand = open(pathname+"\pixel_remap.txt")
...
if arrange_data_by_chip == 0:
	...
	outfilename = "\PH_summmary_bychip2_" +inp+ ".csv"
	#outfilename = "\PH_average_bychip2_" +inp+ ".csv"
try:
...
if arrange_data_by_chip == 0:
	...
	put_channels_in_order(outhand, outlist)
