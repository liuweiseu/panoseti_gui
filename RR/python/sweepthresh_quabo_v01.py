#PANOSETI Quadrant Board Threshold Sweep
#Run this code in a command window to permit sweeping one of the threshold DACs
#After each step, the prpgram will write the new thresh value to a handshake file, to tell
#  another program to take and store a packet
#v01 November 26, 2018
#   Started with control_quabo_v03
...
while True:
    ...
    inp = input("Enter 3 values: start, stop, step, range 0 to 1023:\n")
    vals = inp.split(',')
    
    if len(vals) != 3: 
        vals  = ['180','220','1']
        print("Using 180, 220, 1")
    Bthresh = int(vals[0])
    Ethresh = int(vals[1])
    Sthresh = int(vals[2])
    for dac_override in range(Bthresh,Ethresh,Sthresh):
        time.sleep(0.1)
        print("Sending DAC value: ", dac_override)
        fhand = open(configfilename)
        send_maroc_params(fhand, dac_override)
        fhand.close()
        fhand_hs = open(handshake_filename, 'w')
        fhand_hs.write("THR= "+ str(dac_override))
        fhand_hs.close()
