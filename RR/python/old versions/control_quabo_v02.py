...
# The Quabo IP address is hard-coded below
# The 'H' SendHousekeeping command turns on the periodic housekeeping update, but the
# housekeeping packets are sent to a different UDP port, so won't be seen here.
# Run hk_log_quabo.py in a separate command window to see housekeeping
#v01 Oct 17 2018 RR
...
while True:
    inp = input('''Enter 
    "L" to load all configuration parameters from quabo_config.txt,
    ...
    "H" to report housekeeping data,
    "R" to send a system reset,   
    "ARP" to report ARP table,
    or "q" to quit
    ''')
    # if inp =='X':
        # inp1 = input("lsb_pos, field_width, value")
        # vals = inp1.split(',')
        # set_bits(0, int(vals[0]), int(vals[1]), int(vals[2]))
        # print (MAROC_regs[0])
        
    if inp == 'q':
        sock.close()
        quit()
    # if inp == '':  #LED test for the KC705
        # cmd_payload = bytearray(64)
        # for i in range(64): cmd_payload[i]=0
        # cmd_payload[0] = 0x7f
        # sendit(cmd_payload)
