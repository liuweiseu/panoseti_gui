#PANOSETI Quadrant Board HV Ramp Program
#Run this code in a command window to permit sweeping one of the HV DACs
#After each step, the prpgram will write the new thresh value to a handshake file, to tell
#  another program to take and store a housekeeping packet
#v01 March 28, 2019
...
wait_after_each_setting = 6
wait_after_handshake = 19
HVtestvals = [0,10000, 20000, 30000, 40000, 45000, 46000, 47000, 48000, 49000, 50000, 51000, 52000, 53000, 0]
#HVtestvals = [0,1000, 2000, 3000, 4000, 5000]

which_dac = 1  #can be 0, for no override, or 1 or 2 for DAC1 or DAC2

    
configfilename = ".\config\quabo_config.txt"
#Send to hard-coded quabo address
UDP_DEST_IP = "192.168.1.11"
...
#Pass a DAC override value to this routine
def send_maroc_params(fhand, dac_override):
        ...
        if (tag == "DAC2"): 
            ...
            for i in range (4):
                if which_dac != 2: 
                    ...
                    vals_revbits.append(reverse_bits(dac_override,10))
...
while True:
    ...
    inp = input("Enter to start test\n")
    vals = inp.split(',')
    
    for dac_val in HVtestvals:
        print("Sending DAC value: ", dac_val)
        cmd_payload = bytearray(64)
        for i in range(64): cmd_payload[i]=0
        if (echo_command): cmd_payload[0] = 0x82
        else: cmd_payload[0] = 0x02
        LSbyte = dac_val & 0xff
        MSbyte = (dac_val >> 8) & 0xff
        for chan in range(4):
            cmd_payload[2*chan+2]=LSbyte
            cmd_payload[2*chan+3]=MSbyte
        if connected ==1:
        ...
        fhand_hs = open(handshake_filename, 'w')
        fhand_hs.write("HV= "+ str(dac_val))
        fhand_hs.close()
...
while True:
    ...
    "T" to load the trigger mask values from the config.yxy file,
